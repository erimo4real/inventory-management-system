...
export default class ReportRepository {
  ...
  async getRecentTransactions(siteId, days = 7, limit = 50) {
    const result = await pool.query(`
      ...
      WHERE t.site_id = $1 AND t.created_at >= NOW() - ($2 || ' days')::interval
      ORDER BY t.created_at DESC
      LIMIT $3
    `, [siteId, String(days), limit]);
    return result.rows;
  }
  ...
  async getMonthlyTrends(siteId, months = 6) {
    const result = await pool.query(`
      ...
      FROM inventory_transactions
      WHERE site_id = $1 AND created_at >= NOW() - ($2 || ' months')::interval
      GROUP BY DATE_TRUNC('month', created_at)
      ORDER BY month ASC
    `, [siteId, String(months)]);
    return result.rows;
  }
